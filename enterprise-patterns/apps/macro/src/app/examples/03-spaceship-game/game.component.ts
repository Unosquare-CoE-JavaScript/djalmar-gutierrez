...
export class GameComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.leftArrow$ = fromEvent(document, 'keydown').pipe(
      // SOMETHING GOES HERE!
      filter((eve: KeyboardEvent) => eve.key === 'ArrowLeft'),
      mapTo((currPos) => this.increment(currPos, 'x', -10))
    );

    this.rightArrow$ = fromEvent(document, 'keydown').pipe(
      // SOMETHING GOES HERE!
      filter((eve: KeyboardEvent) => eve.key === 'ArrowRight'),
      mapTo((currPos) => this.increment(currPos, 'x', 10))
    );

    merge(this.leftArrow$, this.rightArrow$).pipe(
      startWith(this.shipPosition),
      // SOMETHING GOES HERE!
      scan((acc: Coordinate, curr: Function) => curr(acc))
    )
